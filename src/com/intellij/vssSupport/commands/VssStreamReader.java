...
public final class VssStreamReader implements Runnable {
  ...
  public String getReadString() {
    if (myOutput == null) {
      try {
        myOutput = StringUtil.convertLineSeparators(
          myByteContents.toString(EncodingProjectManager.getInstance(myProject).getDefaultCharset().name()));
      }
  ...
  private void checkTextAvailable() {
    ...
    else if (text.indexOf(NO_DATABASE_MESSAGE) != -1) {
      reason = VssBundle.message("exception.text.no.database.found");
    }
    else if (text.indexOf(NO_ACCESS_RIGHTS_MESSAGE) != -1) {
